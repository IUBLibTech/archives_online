...
namespace :archives_online do
  ...
  task delete_ead: :environment do
    ...
    puts "archives_online-Arclight deleted #{filename} from solr index (in #{solr_elapsed_time.round(3)} secs).\n"

    postgres_elapsed_time = Benchmark.realtime do
      EadProcessor.remove_ead_from_db(filename)
    end

    puts "archives_online-Arclight deleted #{filename} from postgres database (in #{postgres_elapsed_time.round(3)} secs).\n"
  end
end
