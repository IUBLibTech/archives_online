BotChallengePage.configure do |config|
  ...
  enabled = ENV['CF_BOT_ENABLED'] || false
  config.enabled = enabled&.to_s == 'true' ? true : false
  ...
  config.cf_turnstile_sitekey = ENV['CF_BOT_SITE_KEY'] || "1x00000000000000000000AA"
  # Always pass testing secret_key: "1x0000000000000000000000000000000AA"
  config.cf_turnstile_secret_key = ENV['CF_BOT_SECRET_KEY'] || "1x0000000000000000000000000000000AA"
  ...
  duration = ENV['CF_BOT_DURATION'] || 600
  config.session_passed_good_for = duration.to_i.minutes
